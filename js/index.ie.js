...
var STORAGE_USEDARKTHEME = "useDarkTheme";
var STORAGE_WIDTH = "table_width";
var STORAGE_HEIGHT = "table_height";
var DEFAULT_LAYOUT_WIDTH = 2;
var DEFAULT_LAYOUT_HEIGHT = 2;
var STORAGE_TIMEZONE = "timezone";
var STORAGE_INTERVAL = "interval";
var STORAGE_SHOWDETAILS = "showDetails";
var STORAGE_SHOWBOTTOMTOOLBAR = "showBottomToolbar";
var STORAGE_SHOWTOPTOOLBAR = "showTopToolbar";
var STORAGE_ALLOWSYMBOLCHANGE = "allowSymbolChange";
var STORAGE_USESMALLBUTTON = "useSmallButton";
var STORAGE_CHARTSPAIRS = "charts.pairs";
var BASEURL_COINIGY = "https://www.coinigy.com/main/markets";
...
function prepareCharts() {
    ...
    var elementWidth = document.getElementById("box-width");
    ...
    var elementHeight = document.getElementById("box-height");
...
function removeChart(boxElement, chartPairs) {
    ...
    charts.pairs.splice(charts.pairs.findIndex(function (x) {
        return x == chartPairs;
    }), 1);
...
function createChart(chartTicker) {
    ...
    var topButtonContainerElement = document.createElement("div");
...
function loadCharts(url) {
    ...
    if (charts.pairs.length == 0) {
        ...
        if (chartPairs) {
            ...
            charts.pairs.push("BINANCE:BTCUSDT");
            charts.pairs.push("BINANCE:ETHUSDT");
            charts.pairs.push("BINANCE:MATICUSDT");
            charts.pairs.push("BINANCE:DOTUSDT");
            charts.pairs.push("BINANCE:SOLUSDT");
            charts.pairs.push("BINANCE:SHIBUSDT");
        }
    }
}
...
function bindInputKeyUp() {
    ...
    document.getElementById("SinglepairsInput").addEventListener("keyup", function (event) {
        event.preventDefault();
        if (event.keyCode === 13) {
            chartTicker = this.value.toUpperCase();
...
//
function viewDIV() {
    var viewStr = arguments.length > 0 && arguments[0] !== undefined ? arguments[0] : "";
    var showView = arguments.length > 1 && arguments[1] !== undefined ? arguments[1] : true;

    showView = showView ? "block" : "none";
    document.getElementById(viewStr).style.display = showView;
}
function storeMAN() {
    var add_sub = arguments.length > 0 && arguments[0] !== undefined ? arguments[0] : true;
    var itemName = arguments.length > 1 && arguments[1] !== undefined ? arguments[1] : "";
    var itemValu = arguments.length > 2 && arguments[2] !== undefined ? arguments[2] : "";

    if (gbl_isStorageUsable) {
        try {
            if (add_sub) localStorage.setItem(itemName, itemValu); else return localStorage.getItem(itemName);
            return true;
        } catch (error) {
